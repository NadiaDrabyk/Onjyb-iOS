...
@interface CompletedTaskVC ()

@end

@implementation CompletedTaskVC
...
{
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"completedTaskCell"];
    
    
    if (cell == nil)
    {
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"completedTaskCell"];
    }
    ...
    if (mAryTaskList && mAryTaskList.count)
    {
        ...
        else
        {
            ...
            [lblOverTime setFont:[UIFont boldSystemFontOfSize:15.f]];
        }
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString * strApprove = [dicCell valueForKey:global_key_approve_status];
    if ([strApprove isEqualToString:global_value_reject]) {
        UIStoryboard *mainStoryboard = [UIStoryboard storyboardWithName:@"Main" bundle: nil];
        RegisterTimeDetailsVC * vc = [mainStoryboard instantiateViewControllerWithIdentifier: @"RegisterTimeDetailsVC"];
        vc.dicInfo = [dicCell mutableCopy];
...
- (void) getTaskList {
    ...
    [param setObject:@"approve,reject" forKey:global_key_approve_status];
